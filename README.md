...
> Database-Inspector - Open source tool by AetherCodeHQ

`cli` `database` `developer-tools` `golang` `inspection`

## What is it?

**Database Inspector** is a data tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
go build -o Database-Inspector .
```

### Run
```bash
./Database-Inspector --help
```
...
```bash
# Basic usage
./Database-Inspector --path ./target

# With options
./Database-Inspector --path ./target --format json --output report.json

# Verbose mode
./Database-Inspector --path ./target --verbose
```
...
### Basic scan
```bash
./Database-Inspector --path ./my-project
```

### JSON report
```bash
./Database-Inspector --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Database Inspector
  run: ./Database-Inspector --path . --format json --output report.json
```
